...
# With namespace
npx claude-flow@alpha sparc run supabase-admin "your task" --namespace supabase-admin

# Non-interactive mode
npx claude-flow@alpha sparc run supabase-admin "your task" --non-interactive
```

### Option 3: Local Installation
```bash
# If claude-flow is installed locally
./claude-flow@alpha sparc run supabase-admin "create user authentication schema"
```
...
```bash
# Store mode-specific context
npx claude-flow@alpha memory store "supabase-admin_context" "important decisions" --namespace supabase-admin

# Query previous work
npx claude-flow@alpha memory query "supabase-admin" --limit 5
```
